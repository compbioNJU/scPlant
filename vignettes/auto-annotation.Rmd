...
```{r}
AutoAnnotate_SingleR(SeuratObj, ref, ref_type = "single-cell")
```

```{r}
AutoAnnotate_Celaref(SeuratObj, counts_ref, cellinfo_ref)
```
...
```{r}
AutoAnnotate_scCATCH(SeuratObj, marker_custom)
```

```{r}
AutoAnnotate_Garnett(SeuratObj, marker_file_path, gene_id_type, species = 'Ath')
```

```{r}
AutoAnnotate_CellAssign(SeuratObj, marker_list)
```
