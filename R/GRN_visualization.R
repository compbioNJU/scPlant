...
ras_exp_hmp <- function(SeuratObj, rasMat, group.by = "seurat_clusters", assay = 'SCT') {
  ...
  # mean TF expression
  Idents(SeuratObj) <- SeuratObj@meta.data[[group.by]]
...
ras_exp_scatter <- function(SeuratObj, rasMat, gene, reduction = 'umap') {
  ...
  SeuratObj$Regulon <- rasMat[Cells(SeuratObj), gene]
...
#' 
#' @importFrom igraph graph_from_data_frame E V 
#'
#' @export
#'
topRegulons <- function(rssMat, topn = 5) {
  ...
  E(net)$arrow.size <- .2
  E(net)$arrow.width <- .2
  V(net)$label.cex <- ifelse(V(net)$name %in% celltypes, 0.6, 0.3)
...
ToptargetsofTopregulons <- function(rssMat, 
  ...
  set.seed(123)
  plot(net, layout=layout_with_kk, vertex.label.degree=0, vertex.label.dist=0, vertex.pie=pie.values, 
...
SpecificityRank <- function(rssMat, cluster, topn = 10) {
  data <- tibble::enframe(sort(rssMat[, cluster], decreasing = T)) %>% 
  ...
  pp <- ggplot(data, aes(index, RSS)) + 
    ...
    ggrepel::geom_text_repel(inherit.aes = FALSE, data = data.label, aes(index, RSS, label=regulon), size=4,
                             max.overlaps = 30) + 
    ggtitle(cluster) + ylab("Specificity score") + xlab('Regulons') +
