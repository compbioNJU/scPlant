...
RunCytoTRACE <- function(SeuratObj,
  ...
  if (keep_orig_reduction) {
    if (save_plot) {
      if (!dir.exists(paths = "./output/CytoTRACEoutput")) {
        dir.create("./output/CytoTRACEoutput", recursive = TRUE)
      }
      saveRDS(results, './output/CytoTRACEoutput/cytotrace_result.rds')
      CytoTRACE::plotCytoTRACE(results, outputDir = "./output/CytoTRACEoutput/", emb = Seurat::Embeddings(SeuratObj, reduction=reduction_method))
      CytoTRACE::plotCytoGenes(results, numOfGenes = 10, outputDir = "./output/CytoTRACEoutput/")
    }
    ...
    if (save_plot) {
      ...
      saveRDS(results, './output/CytoTRACEoutput/cytotrace_result.rds')
      CytoTRACE::plotCytoTRACE(results, outputDir = "./output/CytoTRACEoutput/")
      CytoTRACE::plotCytoGenes(results, numOfGenes = 10, outputDir = "./output/CytoTRACEoutput/")
    }
